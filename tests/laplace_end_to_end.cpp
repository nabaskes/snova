#include <vector>
#include <tuple>
#include "../fin_diffs/poisson_eq/laplace_eq.cpp"
...
int main() {
  ...
  BoundaryRow bot_row (rows, cols, 255, rows-1);
  BoundaryColumn left_col (rows, cols, 255, 0);
  BoundaryColumn right_col (rows, cols, 255, cols-1);
  ...
  LaplaceSimulator sim (rows, cols, boundary_conds, init_conds);
  sim.calculate(1000, "end_to_end", print_times);
}
